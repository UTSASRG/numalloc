...
Some memory allocators support the NUMA architecture~\cite{tcmallocnew, kim2013node, yang2019jarena, mimalloc}, as further discussed in Section~\ref{sec:related}. 
%They tried to reduce remote accesses, but with different requirements. For instance, JArena requires the change of applications and OS~\cite{yang2019jarena}, while nMART requires the change of the OS~\cite{kim2013node}.  
...
%Fourth, it does not take advantage of new hardware feature like huge page support. 
 ...
 % explicit huge page management based on allocation/deallocation pattern. 
