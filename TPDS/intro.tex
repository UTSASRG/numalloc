...
%These NUMA allocators aim to reduce remote accesses
...
%, as threads can be migrated between different nodes. 
...
%As a result, 
%none of existing allocators achieves the full locality of memory allocations, where a thread may still allocate a remote object that is locating on a different node from it is currently running on. This problem is mainly caused by the fact that a thread can be migrated among different physical nodes. In theory, an allocator can still avoid remote allocations if it could check the physical node of each thread and make sure that a thread only gets a memory block.  before each allocation,  either the memory allocator should check the origin
%the expensiveness of identifying a thread's location or a

%Based on our investigation, the major reason of this problem is due to thread migrations. 

%that will maintain objects from the same thread. 
