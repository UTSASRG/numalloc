...
Overall, \NM{} has the best performance among these allocators. In particular, \NM{} is 14.8\% faster than the second-best allocator (mimalloc) and 17.1\% faster than the default allocator. %\todo{explain why mimalloc is good? why other allocators are not good}
...
% We utilize the \texttt{perf} to collect the number of remote accesses (both load and store) on these applications when they are running with all allocators. 
