...
#define ALIGN_TO_CACHELINE(size) (size % 64 == 0 ? size : (size + 64) / 64 * 64)
  ...
  static void bindMemoryToNode(void * ptr, size_t size, int nodeindex) {
    ...
    if(mbind(ptr, size, MPOL_BIND, &mask, NUMA_NODES+1, 2) == -1) {
  ...
  static void * mmapPageInterleaved(size_t sz, void * startaddr = NULL) {
    void * ptr;
    ...
    // Set the memory to be interleaved.
  ...
  static void * mmapBlockInterleaved(size_t sz, void * startaddr) {
