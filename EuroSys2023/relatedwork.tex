...
%: continuous memory allocations from different threads will be satisfied from different huge pages. This is 
...
%\todo{That is, small allocations before a medium/large allocation (e.g., larger than 1MB) will be satisfied from small pages. }    
Instead, \NM{} allows the sharing of huge pages from different threads 
% and objects with different size classes
...
%We believe that the major reason is caused by allowing different threads to share the same huge page. 
%all memory (except the interleaved heap) from huge pages,  but reducing the fragmentation by making different size classes to share the same huge page. 
\NM{} could be further improved by borrowing some sophisticated mechanisms of LLAMA and TEMERAIRE in the future. 
%In theory, \NM{} may introduce higher overhead, as it utilize huge pages for all memory objects, preventing the returning of huge pages back to the OS. 
...
\begin{comment}
\todo{
snmalloc: A Message Passing Allocator (ISMM ’19)
Timescale Functions for Parallel Memory Allocation (ISMM ’19)

Poseidon: Safe, Fast and Scalable Persistent Memory Allocator (Middleware '20)

POSTER: Understanding and Optimizing Persistent Memory Allocation (PPoPP '20)


HALO: Post-Link Heap-Layout Optimisation (CGO ’20)

A lock-free coalescing-capable mechanism for memory management (ISMM 2019)
