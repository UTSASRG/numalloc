...
% \NM{} will use node-interleaved binding by default, as it has a better performance based on our evaluation. But users can switch to the node-saturate binding by controlling the environment variable. 
...
% First, \NM{} classifies each allocation by it size, limiting huge pages only for big objects. Second, 
...
%     \caption{Performance of different allocators on PARSEC, OpenMP/MPI and real applications, where all data\\ is normalized to that of the default Linux allocator. Here, a lower bar indicates a better performance. \\ Note that some applications are failed to run with some allocators.
...
%  \begin{figure*}[!ht]
%     \centering 
%     \includegraphics[width=7in]{figure/remote-access-new-format.jpg}
%     \caption{Normalized runtime, remote accesses, and TLB misses of different allocators (\todo{first normalized to \NM{} and then do logarithmic transformation), where the lower is the better.} }
