...
(2) thread binding eliminates remote accesses caused by the above-mentioned thread migrations, but it does not exclude OS-based scheduling \todo{as \NM{} only binds a thread to a specific node, instead of a core. Moreover, \NM{} allows users to specify the binding explicitly;}
...
\todo{In contrast, \NM{} includes explicit thread binding which makes it possible to share the pages between threads running on the same node. This sharing also applies to objects of different size classes, which further reduces the memory fragmentation.}
% We call it ``incremental sharing'' as each thread only fetches the amount of memory it needs from currently available page at a time, instead of the entire page.
