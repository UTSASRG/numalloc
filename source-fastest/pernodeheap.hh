...
class PerNodeHeap {
  private:
    ...
    PerNodeSizeClass  _sizes[SMALL_SIZE_CLASSES];
    int   _numClasses;
    int   _nodeindex;
 ...
 public:
   size_t computeMetadataSize(size_t heapsize) {
     size_t size = 0;
      ...
      size += _bigObjects.computeImplicitSize(heapsize);
  ...
  char * allocateOnemb(size_t size) {
    ...
    ptr = allocateFromFreelist(SIZE_ONE_MB); 

    // If there is no freed bigObjects, getting one from the bump pointer
    if(ptr == NULL) {
      ptr = allocateFromBumppointer(SIZE_ONE_MB);
    } 

    // Mark the size information
    markPerMBInfo(ptr, SIZE_ONE_MB, size);
